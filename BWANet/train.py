...
import argparse
parser = argparse.ArgumentParser(description='Hyper-parameters for BWANet')
...
model_restored = BWANet(inp_channels=3,out_channels=3,dim = 16,num_blocks = [2,2,4],bias = False,LayerNorm_type = 'WithBias',attention=True,skip = True)
...
for epoch in range(start_epoch, OPT['EPOCHS'] + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        loss2 = ls(restored, target)
        loss3 = lp(restored, target)

        loss=loss1 + 0.05*loss2 + 0.1*loss3

        # Back propagation
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model_restored.parameters(), max_norm=0.001)
        optimizer.step()
